...
var (
	...
	commandHelpTextPollSettingMultiVote = &i18n.Message{
		...
		Other: "Allow users to vote for X options. Default is 1. If X is 0, users can vote for all options even after adding options.",
	}
	...
	elements = append(elements, model.DialogElement{
		...
		HelpText: p.bundle.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				...
				Other: "The number of options that an user can vote on. 0 is meaning that users can vote for all options even after adding options.",
			}}),
		Optional: false,
	})
	elements = append(elements, model.DialogElement{
